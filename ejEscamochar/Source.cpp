...
// Se realizan dos bucles de X e Y iteraciones, respectivamente. Como siempre va a ser X+Y = longitud, podemos decir
// que se dan del orden de O(n) iteraciones, cada una de coste constante

void ListLinkedSingle::escamochar(ListLinkedSingle& dest) {
    Node* prev = nullptr;
    Node* cur = head;

    // Buscar el primer número positivo
    while (cur != nullptr && cur->value < 0) {
        prev = cur;
        cur = cur->next;
    }

    // Si prev es nullptr, significa que la lista comienza con un número positivo
    if (prev == nullptr) {
        // La lista destino queda vacía
        dest.head = nullptr;
    }
    else {
        // Si prev no es nullptr, entonces la lista this empieza en prev->next
        dest.head = head;
        head = cur;
    }

    // Buscar el último número positivo
    ...
    // Dividir la lista en la última posición positiva
    if (last_positive != nullptr) {
        Node* rest = last_positive->next;
        ...
        if (prev != nullptr) {
            ...
            // Si prev es nullptr, la lista destino es rest
            dest.head = rest;
        }
    }
}
